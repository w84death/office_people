			...
			*/
			...
			draw: function() {
				for (var i = 0; i < ig.gui.elements.length; i++) {
					...
					if((state == 'hover' && (ig.input.state('touch') || ig.input.pressed('touch'))) || (ig.input.pressed(element.keybind))) {
						state = 'active';
						if(ig.input.state('touch') && typeof ig.gui.elements[i].mouseDown == 'function')
							ig.gui.elements[i].mouseDown.call(element);
						if(ig.input.pressed('touch') || ig.input.pressed(element.keybind)) {
							// Toggle (click)
							if(element.toggle)
